...
# --- Лингвистические библиотеки (без изменений) ---
try:
    import nltk
...
# ============================================================
# СЛОВАРЬ ЧАСТЕЙ РЕЧИ (без изменений)
# ============================================================
POS_EN_TO_RU = {
...
CONFIG = {
    ...
    'pos_mode': 'wordnet',   # 'wordnet', 'spacy' или None
}

# --- Пути (измените под свои файлы) ---
# Список PDF-файлов, которые нужно обработать
PDF_PATHS = [
    r"C:\Users\kresovivan\Programming\pyspark_project\python-crash-course-introduction-to-programming-3-ed.pdf",
    r"C:\Users\kresovivan\Programming\pyspark_project\mastering-object-oriented-programming-with-python.pdf",
    r"C:\Users\kresovivan\Programming\pyspark_project\data-analysis-with-python-and-pyspark.pdf",
    r"C:\Users\kresovivan\Programming\pyspark_project\automate-the-boring-stuff-with-python-3-ed.pdf"
...
# --- Технические артефакты (без изменений) ---
TECH_PATTERNS = [
    r'^[a-z]+_[a-z_]+$',
    r'^[a-z]+[A-Z][a-zA-Z]*$',
    r'^[a-z]+\.[a-z]+$',
...
# ============================================================
# БЛОК 2: ЧАСТИ РЕЧИ (без изменений)
# ============================================================
...
# ============================================================
# БЛОК 3: ТРАНСКРИПЦИЯ (без изменений)
# ============================================================
...
# ============================================================
# БЛОК 4: КЭШ ПЕРЕВОДОВ (без изменений)
# ============================================================
...
# ============================================================
# БЛОК 5: ИЗВЛЕЧЕНИЕ СЛОВ ИЗ ОДНОГО PDF
# ============================================================
...
def extract_words_from_single_pdf(pdf_path: str, lemmatizer=None, stop_words=None) -> list[str]:
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for i, page in enumerate(pdf.pages, 1):
            ...
            if i % 50 == 0 or i == total_pages:
                print(f"   Страниц {i}/{total_pages} ... ({len(all_words)} слов)")
    print(f"   Извлечено {len(all_words)} сырых слов из {os.path.basename(pdf_path)}")
    return all_words

def extract_words_from_multiple_pdfs(pdf_paths, lemmatizer=None, stop_words=None) -> tuple[list[str], Counter]:
    """Возвращает (отфильтрованный список слов, Counter частот)."""
    all_raw = []
    for path in pdf_paths:
        raw = extract_words_from_single_pdf(path, lemmatizer, stop_words)
        all_raw.extend(raw)
    
    # Лемматизация (общая для всех слов)
    if CONFIG['use_lemmatization'] and lemmatizer:
        print("🔤 Лемматизация всех слов...")
        all_raw = [lemmatizer.lemmatize(w) for w in all_raw]
    ...
    print(f"\n📊 После фильтрации: {len(filtered):,} слов")
    if skipped_stats:
        print(f"   Пропущено: {dict(skipped_stats)}")
    
    word_freq = Counter(filtered)
    return filtered, word_freq

# ============================================================
# БЛОК 6: РАБОТА С CSV (без изменений)
# ============================================================
        ...
        with open(csv_path, 'r', encoding='utf-8-sig') as f:
            ...
            for row in reader:
                if 'English Term' in row and row['English Term']:
                    ...
                    existing.add(word)
        if existing:
            print(f"📂 Загружено {len(existing)} слов из существующего CSV")
        else:
...
def main():
    print("=" * 70)
    print("📚 PDF → CSV: Обработка нескольких книг (словарь с IPA, транскрипцией, частями речи)")
    print("=" * 70)
    ...
    # 1. Извлечение слов из всех PDF
    all_words_filtered, word_freq = extract_words_from_multiple_pdfs(PDF_PATHS, lemmatizer, stop_words)
    if not all_words_filtered:
        print("❌ Не удалось извлечь слова из PDF")
        return

    unique_words = sorted(word_freq.keys())
    print(f"\n📊 СТАТИСТИКА ПО ВСЕМ КНИГАМ:")
    print(f"   Всего слов (с повторами): {len(all_words_filtered):,}")
    print(f"   Уникальных слов:          {len(unique_words):,}")
    print(f"   Средняя частотность:      {len(all_words_filtered)/len(unique_words):.1f}")
    ...
    if existing:
        ...
        if choice == 'q':
            ...
            mode = 'w'
            print("🔄 Полная перезапись CSV со всеми словами из всех книг.")
        else:
            words_to_process = new_words
            mode = 'a'
            print(f"➕ Будет добавлено {len(words_to_process)} новых слов.")
    else:
        words_to_process = unique_words
        mode = 'w'
        print("\n🆕 Создаём новый CSV")
    ...
    try:
        for idx, word in enumerate(words_to_process, 1):
            ...
            if idx % 10 == 0 or idx == len(words_to_process):
                print(f"   {idx}/{len(words_to_process)}: {word} ({lemma}) | {ipa} | POS={pos_en or '??'} | {translation[:25] if translation else '...'}")
